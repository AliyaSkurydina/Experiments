...
\begin{frame}
	\begin{itemize}
		...
		\item регуляризованный метод минимальных невязок (ММН):
		$$\min_{\beta}\{\|A'(u^k)(u^k-\beta(A(u^k)-f_{\delta})-F(u^k)\|^2\},$$$$ u^{k+1} =u^k - \frac{\langle B(u^k)S_\alpha(u^k), S_\alpha (u^k)\rangle}{\|B(u^k)S_\alpha(u^k)\|^2}(A(u^k)+\alpha(u^k-u^0)-f_\delta),$$
		где $B(u^k)=A'(u^k)+\bar{\alpha}I, \quad S_\alpha(u^k)=A(u^k)+\alpha(u^k-u^0)-f_\delta$.
	\end{itemize}
\end{frame}
...
%	$\|u_{\alpha(\delta)}^{\delta, k}-u_{\alpha(\delta)}^{\delta}\|\le q^k(\delta)r$
	В работе$^*$ (теорема 5.1., Васин) приводится оптимальная по порядку оценка погрешности двухэтапного метода с монотонным оператором:
	\begin{equation*}
...
\begin{frame}{2.3. Оценка невязки двухэтапного метода с немонотонным оператором}
	...
	{\scriptsize
		\let\thefootnote\relax\let\thefootnote\relax\footnotetext{\footnotesize 8. В. В. Васин, А. Ф. Скурыдина. Двухэтапный метод построения регуляризующих алгоритмов для нелинейных некорректных задач // Труды ИММ УрО РАН Т.23 В.1 (2017), С. 57–74.}}
\end{frame}
