...
В дальнейшем получили развитие устойчивые (регулярные) методы решения нелинейных некорректных задач на основе принципа итеративной регуляризации и иных подходах в работах А.~Б.~Бакушинского, Б.~Т.~Поляка, А.~В.~Гончарского, М.~Ю.~Кокурина, В.~В.~Васина, В.~Г.~Романова, С.~И.~Кабанихина, Ф.~П.~Васильева, В.~А.~Морозова, А.~Г.~Яголы, А.~С.~Леонова, А.~Л.~Агеева, В.~П.~Тананы, В.~И.~Максимова, А.~И.~Короткого, А.~Б.~Смирновой, A.~Neubauer, B.Kaltenbacher, H.~W.~Engl, M.~Hanke, C.~Boeckmann.

Структурные задачи гравиметрии и магнитометрии --- важный класс нелинейных некорректных задач. При измерении геофизических полей на большой площади, возникает необходимость решать системы нелинейных уравнений большой размерности с использованием параллельных вычислений.

В ИГФ УрО РАН разработана методика и алгоритмы решения обратных задач на основе метода локальных поправок (И.~Л.~Пруткин, П.~С.~Мартышко, И.~В.~Ладовский, А.~Г.~Цидаев).  

В ИММ УрО РАН разработаны параллельные алгоритмы на основе методов Ньютона, Левенберга -- Марквардта и процессов градиентного типа (В.~В.~Васин, Е.~Н.~Акимова, Л.~Ю.~Тимерханова, Г.~Я.~Пересторонина, В.~Е.~Мисилов).
\end{frame}
...
\begin{frame}{Глава 1. Решение уравнений с монотонным оператором}
	В первой главе рассматриваются методы решения некорректных задач с монотонным оператором. Дано обоснование двухэтапного метода на основе регуляризованного метода Ньютона. Построены методы минимальной ошибки, наискорейшего спуска и минимальных невязок решения нелинейных уравнений и доказывается их сходимость к регуляризованному решению. Приводится оценка погрешности регуляризованного решения.
\end{frame}
...
\begin{frame}{1.1. Регуляризованный метод Ньютона}
		...
		\scriptsize
		\let\thefootnote\relax\let\thefootnote\relax\footnotetext{\footnotesize * А. Б. Бакушинский. Регуляризующий алгоритм на основе метода Ньютона -- Канторовича ... // ЖВМиМФ, 16:6 (1976). С. 1397--1604.}
...
\begin{frame}
	Пусть для монотонного оператора $A$ выполнены условия $$
	\|A(u)-A(v)\|\leqslant N_1\|u-v\|,
	\|A'(u)-A'(v)\|\leqslant N_2\|u-v\|, \quad \forall u, v \in S_r(u^0), \eqno (1.6)$$
...
\begin{frame}{1.2. Нелинейные аналоги альфа-процессов \textbf{(new)}}
	Впервые $\alpha$-процессы для линейного  самосопряженного положительно определенного оператора  были предложены {\color{red}М.~А.~Красносельским и др. (1969)}. \\
	Для нелинейного оператора итерационный процесс запишем в виде:
	$$u^{k+1}=u^k-\beta_k(A(u^k)+\alpha(u-u^0)-f_{\delta}).$$ \\
	Используя разложение Тейлора в точке $u^k$ исходного уравнения, переходим к линейному уравнению в фиксированной итерационной точке, получаем последовательность линейных задач
	$$A'(u^k)u^k=F^k, \quad F^k=f_\delta+A'(u^k)u^k-A(u^k)$$ и параметр $\beta_k$ находим из условия минимума соответствующих функционалов
%	\begin{itemize}
...
\begin{frame}
	\begin{itemize}
		...
		\item регуляризованный метод минимальных невязок (ММН):
		$$\min_{\beta}\{\|A'(u^k)(u^k-\beta(A(u^k)-f_{\delta})-F(u^k)\|^2\},$$$$ u^{k+1} =u^k - \gamma\frac{\langle B(u^k)S_\alpha(u^k), S_\alpha (u^k)\rangle}{\|B(u^k)S_\alpha(u^k)\|^2}(A(u^k)+\alpha(u^k-u^0)-f_\delta),$$
		где $B(u^k)=A'(u^k)+\bar{\alpha}I, \quad S_\alpha(u^k)=A(u^k)+\alpha(u^k-u^0)-f_\delta$, \\$\gamma$ --- демпфирующий множитель.
	\end{itemize}
\end{frame}
...
\begin{frame}{\small 1.2. Оценка скорости сходимости нелинейных $\alpha$-процессов}
		Пусть для монотонного оператора $A$ выполнены условия  $\|A'(u)-A'(v)\|\leqslant N_2\|u-v\|$, $\forall u, v \in S_r(u^0)$,	$\|A'(u^0)\| \leqslant N_0\leqslant N_1$,  $\|u^0-u_\alpha\| \leqslant r \quad(*)$ \\ и {\textbf{\color{blue}$A'(u^0)$ --- самосопряженный оператор, \\для ММО $\alpha \leqslant \bar\alpha, \quad r\leqslant \alpha/8N_2, \quad \bar\alpha \geqslant N_0.$}} 
...
%	$\|u_{\alpha(\delta)}^{\delta, k}-u_{\alpha(\delta)}^{\delta}\|\le q^k(\delta)r$
	В работе$^*$ приводится оптимальная по порядку оценка погрешности двухэтапного метода с монотонным оператором:
	\begin{equation*}
	\|u_{\alpha(\delta)}^{\delta, k}-\hat{u}\|\leqslant 4\sqrt{k_0 \delta},
	\end{equation*}
	где приближение $u_{\alpha(\delta)}^{\delta, k}$ зависит от $\delta$, $\hat{u}$ --- решение исходного уравнения, $k_0=(1+N_2\|v\|/2)\|v\|$ ($u^0-\hat{u}=A'(\hat{u})v$ --- истокообразная представимость решения$^{**}$).
	{ \scriptsize
	\let\thefootnote\relax\let\thefootnote\relax\footnotetext{\footnotesize $^*$ В. В. Васин, А. Ф. Скурыдина. Двухэтапный метод построения регуляризующих алгоритмов для нелинейных некорректных задач // Труды ИММ УрО РАН Т.23 В.1 (2017), С. 57–74.} 
...
%	где $$[F(u)](t)=c_0 e^{\int_{0}^{t}u(\tau)d\tau}$$
	...
	{\textbf{\color{blue}Результаты:}} относительные погрешности методов достигают $\varepsilon$ за 8--9 итераций.
...
\begin{frame}{\small 2.2. Скорость сходимости $\alpha$-процессов с немонотонным оператором}
	\begin{block}{\bf Теорема ~2.2.}
		Пусть выполнены условия (*), для MMO:  $0<\alpha\leqslant\bar\alpha$, $r\leqslant\alpha /6\bar SN_2$, $\bar\alpha \geqslant N_0$, \ MHC: $0<\alpha\leqslant\bar\alpha$, $\quad r\leqslant\alpha /3N_2$,
...
\begin{frame}{\small 2.2. Сходимость модифицированных $\alpha$-процессов с немонотонным оператотором}
	...
	$\|A(u)-A(v)\|\leqslant N_1\|u-v\|, \quad \|A'(u)-A'(v)\|\leqslant N_2\|u-v\|, \quad \forall u, v \in S_r(u^0),$
	$\|A'(u^0)\| \leqslant N_0\leqslant N_1, \quad \|u^0-u_\alpha\| \leqslant r,$
	{\textbf{\color{blue}$A'(u^0)$ --- симметричная матрица, спектр которого состоит из неотрицательных различных собственных значений}},
	$0\leqslant\alpha\leqslant\bar{\alpha},\quad \bar{\alpha}\geqslant N_0$, $r=\alpha/6N_2.$ ($**$)
	\vskip -1 mm	
...
\begin{frame}{2.4. Решение модельной задачи магнитометрии}
	...
	\vskip -2 mm
	\textbf{\color{blue}Результаты.} Число обусловленности $\mu(A'_n(u_n^k))\approx 1.8\cdot 10^7$, спектр неотрицательный, состоящий из различных собственных значений, $\bar\alpha=10^{-2}$, $\alpha = 10^{-4}$, $\gamma=1$, относительная погрешность $\varepsilon < 10^{-2}$. Итерационные методы сходятся за 4--5 итераций, у модифицированных меньше время счета.
...
\end{frame}
\begin{frame}{3.5. Задача 1 (сравнение методов решения задачи гравиметрии в двухслойной среде)}
