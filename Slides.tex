...
\begin{frame}{\small 1.1. Оценка скорости сходимости РМН с монотонным оператором}
	\begin{block}{\bf Теорема ~1.1.} 
		...
		$$\|A(u)-A(v)\|\leqslant N_1\|u-v\|, \quad \forall u, v \in S(u^0;R),$$ 
...
\begin{frame}
	...
	\smallskip
	{\textbf{\color{blue}$A'(u^0)$ --- самосопряженный оператор}}, $$  
	...
	\begin{block}{\bf Теорема ~1.2.}
		...
		удовлетворяет неравенству сильной фейеровости, для итераций $u^k$ справедливо соотношение (1.5) и имеет место сходимость
...
\begin{frame}{Модифицированные варианты на основе $\alpha$-процессов}
...
\begin{frame}{\small 1.2. Оценка скорости сходимости нелинейных $\alpha$-процессов}
		Пусть для монотонного оператора $A$ выполнены условия \\ $\forall u, v \in S(u^0;R)$ $\|A(u)-A(v)\|\leqslant N_1\|u-v\|$, $\|A'(u)-A'(v)\|\leqslant N_2\|u-v\|$,	\\ $\|A'(u^0)\| \leqslant N_1$, и $A'(u^0)$ --- самосопряженный оператор,  $\alpha \leqslant \bar\alpha$, $\bar\alpha \geqslant N_1$, \\для ММО $\|u^0-u_\alpha\| \leqslant r$, $r\leqslant \alpha/8N_2.$
...
%	$\|u_{\alpha(\delta)}^{\delta, k}-u_{\alpha(\delta)}^{\delta}\|\le q^k(\delta)r$
	Приводится оптимальная по порядку оценка погрешности двухэтапного метода с монотонным оператором:
	\begin{equation*}
...
Имеется оценка:
	...
	\begin{block}{\bf Теорема ~2.1.}
		...
		\|A'(u)-A'(v)\|\leqslant N_2\|u-v\|, \quad \forall u, v \in S(u^0;R),$$
		$$\|A'(u^0)\| \leqslant N_1.$$
		{\textbf{\color{blue}$A'(u^0)$ --- симметричная матрица, $0<\alpha\leqslant\bar\alpha$, $\bar\alpha\geqslant 4N_0$, $\|u^0-u_\alpha\| \leqslant r$, $r\leqslant\alpha/8N_2\bar S$}}. Тогда для метода (1.3) справедливо заключение теоремы (1.3), где
...
\begin{frame}{\small 2.2. Скорость сходимости $\alpha$-процессов с немонотонным оператором}
	\begin{block}{\bf Теорема ~2.2.}
		Пусть выполнены условия (пред. слайд), $0<\alpha\leqslant\bar\alpha$, $\|u^0-u_\alpha\| \leqslant r$, для MMO: $r\leqslant\alpha /6\bar SN_2$, $\bar\alpha \geqslant N_0$, \ MHC: $0<\alpha\leqslant\bar\alpha$, $\quad r\leqslant\alpha /3N_2$,
...
\begin{frame}{\small 2.2. Сходимость модифицированных $\alpha$-процессов с немонотонным оператотором}
	...
	$\|A(u)-A(v)\|\leqslant N_1\|u-v\|, \quad \|A'(u)-A'(v)\|\leqslant N_2\|u-v\|, \quad \forall u, v \in S(u^0;R),$
	$\|A'(u^0)\| \leqslant N_0\leqslant N_1,$
	{\textbf{\color{blue}$A'(u^0)$ --- симметричная матрица, спектр которого состоит из неотрицательных различных собственных значений}},
	$0<\alpha\leqslant\bar{\alpha},\quad \bar{\alpha}\geqslant N_0$, $\|u^0-u_\alpha\| \leqslant r$, $r\leqslant\alpha/6N_2$.
	\vskip -1 mm	
...
\begin{frame}{3.2. Вычислительная оптимизация метода Ньютона}
...
\begin{frame}
	\begin{block}{Замечание 3.2.}
        ...
        В формуле $\eqref{equ_rmn}$ вместо матрицы $A'(u^k)=\{a_{i,j}\}$ используется диагональная матрица $\overline{A'}=\{\overline{a_{i,j}}\}, \quad \overline{a_{i,j}}=
\begin{cases}
...
\begin{frame}{3.3. Покомпонентный метод типа \\ Левенберга -- Марквардта}
	\vskip -2 mm
	Регуляризованный метод Левенберга -- Марквардта %\textbf{\color{red}(Васин, 2012)}
