...
%	$\|u_{\alpha(\delta)}^{\delta, k}-u_{\alpha(\delta)}^{\delta}\|\le q^k(\delta)r$
	...
	\end{equation*}
	где приближение $u_{\alpha(\delta)}^{\delta, k}$ зависит от $\delta$, $\alpha$, $u_{\alpha(\delta)}^{\delta}$ --- решение регуляриованного уравнения, $k_0=(1+N_2\|v\|/2)\|v\|$ ($u^0-\hat{u}=A'(\hat{u})v$ --- истокообразная представимость решения).
	\let\thefootnote\relax\let\thefootnote\relax\footnotetext{\footnotesize $^*$ В. В. Васин, А. Ф. Скурыдина. Двухэтапный метод построения регуляризующих алгоритмов для нелинейных некорректных задач // Труды ИММ УрО РАН Т.23 В.1 (2017), С. 57–74.} 
\end{frame}
...
Полагая $\Delta u^k(x,y)=\Delta u^k(x',y')=const$ относительно переменных интегрирования,
